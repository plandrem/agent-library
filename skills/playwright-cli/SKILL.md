...
## ACC Sandbox Constraints

When running under Apple Claude Code (ACC), the spawned process sandbox restricts network binding to `localhost:*`. This affects one command:

- **NEVER use `eval`** — it tries to `listen` on `0.0.0.0`, which the sandbox blocks with `EPERM`. Worse, the failure **permanently corrupts the daemon session**, breaking all subsequent commands until you `kill-all` and re-`open`.

Use `run-code` as a drop-in replacement:

```bash
# BROKEN — poisons session:
playwright-cli -s=session eval '() => window.location.href'
playwright-cli -s=session eval '() => document.title'

# SAFE — equivalent results:
playwright-cli -s=session run-code '(page) => page.evaluate(() => window.location.href)'
playwright-cli -s=session run-code '(page) => page.title()'
```

All other commands (`open`, `snapshot`, `click`, `fill`, `press`, `select`, `goto`, `console`, `close`, `run-code`) work without issue.
